...
public class SmaConverterMeasurementDevice extends AbstractMeasurementDevice<SmaConverterConfig> {
   ...
   */
  public SmaConverterMeasurementDevice(MeasureCache inCache, SmaConverterConfig inConfig) {
  ...
  private ModbusClient connect() {
    ...
    String lIpAddress = getConfig().getIp_address();
    Integer lPort = getConfig().getPort();
    int lModbusID = getConfig().getConverter_id();
    ...
    try {
      ...
      log.error(MarkerFactory.getMarker(getId()),
          "SMA converter [modbus id {}] could not be connected: {}:{} - reason: {}", lModbusID,
          lIpAddress, getConfig().getPort(), t.getMessage());
      return null;
    } // catch

    outClient.setConnectionTimeout(getConfig().getTimeout());
    outClient.setUnitIdentifier((byte)Byte.valueOf((byte)lModbusID));

    if (outClient.isConnected()) {
      log.debug(MarkerFactory.getMarker(getId()),
          "SMA converter [modbus id {}] was successfully connected: {}:{}", lModbusID, lIpAddress,
          lPort);
    }
    else {
  ...
  private String getValueFromSMA(Measure inMeasure) {
    ...
    try {
      ...
      log.warn(MarkerFactory.getMarker(getId()),
          "Could not read [{} at {}/{} ({})] from modbus: {}", inMeasure.name(), inMeasure.start,
          inMeasure.length, inMeasure.type.name(), e.getMessage());
      return outValue;
    }
    finally {
      if (lClient != null) {
    ...
    if (lRegisters != null) {
      ...
      if (inMeasure.type == DataType.S32) {
        outValue = String
            .valueOf(ModbusClient.ConvertRegistersToDouble(lRegisters, RegisterOrder.HighLow));
      }
        ...
        outValue = String
            .valueOf(ModbusClient.ConvertRegistersToLong(lRegisters, RegisterOrder.HighLow));
      } // else if
    } // if
  ...
  protected void switchOnCustom() {
    ...
    Float lSalesPerKWh = Float.valueOf(getConfig().getSalesper_kwh());

...
public class SmaConverterMeasurementDevice extends AbstractMeasurementDevice {
  ...
  private ModbusClient connect() {
    ...
    try {
      ...
      // TODO Logging
      // LogHelper.logError(this, "Modbus connection to [{0}:{1}] failed - reason: {2}", lIpAddress,
      // config.getPort(),
      // t.getMessage());
      return null;
    } // catch
    ...
    if (outClient.isConnected()) {
      // TODO Logging
      // LogHelper.logInfo(this, "SMA converter [ID={0}] successfully connected: ip={1}, port={2}",
      // lModbusID, lIpAddress, lPort);
    }
    else {
      // TODO Logging
      // LogHelper.logWarning(this,
      // "SMA converter [ID={0}] could not be connected: ip={1}, port={2} - unknown reason",
      // lModbusID, lIpAddress, lPort);
      outClient = null;
    } // else

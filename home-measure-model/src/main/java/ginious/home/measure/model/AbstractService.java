...
public abstract class AbstractService implements Service {
  ...
  /**
   * Performs the initialization of included measures for this service based on the underlying property
   * <code>service.NAME.included-measures</code>. Measures in the form <code>DEVICE_ID.MEASURE_ID</code> (or
   * <code>DEVICE_ID.*</code> for all measures of a device) must be provided in comma separated form.
   * When the property is empty all devices and all measures are included.
   * 
  ...
  protected void initIncludedMeasures(String inMeasureIdsCsv) {
    ...
    if (lDeviceMeasuresToInclude != null) {
      ...
      for (String lCurrDeviceMeasure : lDeviceMeasuresToInclude) {
        Validate.isTrue(lCurrDeviceMeasure.contains("."), "The device measure ["
            + lCurrDeviceMeasure
  ...
  private void measureChanged(Measure inChangedMeasure) {
    ...
    if (inChangedMeasure.getValue() == null
        || "null".equalsIgnoreCase(inChangedMeasure.getValue())) {
      return;
    } // if

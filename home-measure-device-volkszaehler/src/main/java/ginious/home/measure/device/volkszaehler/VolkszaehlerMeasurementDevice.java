...
public class VolkszaehlerMeasurementDevice extends AbstractMeasurementDevice<VolkszaehlerConfig> {
   ...
   */
  public VolkszaehlerMeasurementDevice(MeasureCache inCache, VolkszaehlerConfig inConfig) {
  ...
  private SerialReceiver getReceiver() {
    ...
    if (receiver == null) {
      ...
      try {
        SerialPort lPort = SerialPortBuilder.newBuilder( //
            getConfig().getDevice()) //
            .setBaudRate(getConfig().getBaudrate()) //
    ...
    log.info(MarkerFactory.getMarker(getId()), "Connected to device [{}]", getConfig().getDevice());
    ...
    for (SmlMessage lCurrMessage : lMessages) {
      ...
      if (lCurrMessage.getMessageBody().getTag() == EMessageBody.GET_LIST_RESPONSE) {
        ...
        for (SmlListEntry lCurrEntry : lListEntries) {
          ...
          if (StringUtils.equals(objName, getConfig().getMeter_ht())) {
            lMeasureToChange = Measure.HT;
          }
          else if (StringUtils.equals(objName, getConfig().getMeter_nt())) {
            lMeasureToChange = Measure.NT;
          }
          else if (StringUtils.equals(objName, getConfig().getMeter_solar())) {
            lMeasureToChange = Measure.SOLAR_OUT;
          }
          else if (StringUtils.equals(objName, getConfig().getMeter_total())) {
            lMeasureToChange = Measure.TOTAL;
          }
          else if (StringUtils.equals(objName, getConfig().getMeter_actual())) {
